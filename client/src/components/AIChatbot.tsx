...
export const AIChatbot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome',
      sender: 'ai',
      text: 'Xin chào! Tôi là CineBot AI 🤖. Bạn cần tư vấn về bộ phim, diễn viên, đạo diễn hay gợi ý tác phẩm điện ảnh nào hôm nay?',
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 font-sans">
      ...
      {isOpen && (
        <div className="glass-panel-glow w-[360px] sm:w-[400px] h-[520px] rounded-3xl border border-amber-500/40 shadow-2xl flex flex-col overflow-hidden animate-fade-in bg-slate-950/95 backdrop-blur-xl">
          {/* Window Header */}
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-emerald-400 font-medium flex items-center space-x-1">
                  ...
                  <span>Trợ lý Điện ảnh Trực tuyến</span>
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3.5 text-xs">
            {messages.map((msg) => (
              ...
              >
                <div
                  className={`w-7 h-7 rounded-xl flex items-center justify-center flex-shrink-0 text-[10px] ${
                    msg.sender === 'user' ? 'bg-amber-500 text-slate-950 font-bold' : 'bg-slate-800 border border-amber-500/30 text-amber-400'
                  }`}
                >
                  {msg.sender === 'user' ? <UserIcon className="w-3.5 h-3.5" /> : <Bot className="w-3.5 h-3.5" />}
                </div>

                <div className={`max-w-[78%] space-y-1 ${msg.sender === 'user' ? 'text-right' : 'text-left'}`}>
                  <div
          ...
          {messages.length < 3 && (
            <div className="px-3 py-2 border-t border-slate-900 flex flex-wrap gap-1.5 bg-slate-900/30">
              {quickPrompts.map((prompt, idx) => (
                <button
                  key={idx}
                  onClick={() => handleSendMessage(prompt)}
                  className="text-[10px] px-2.5 py-1 rounded-full bg-slate-800/80 hover:bg-amber-500/20 text-slate-300 hover:text-amber-300 border border-slate-700 hover:border-amber-500/30 transition text-left truncate max-w-full"
                >
                  {prompt}
                </button>
              ))}
